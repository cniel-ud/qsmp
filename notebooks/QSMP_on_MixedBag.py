...
# %%
sigma = np.r_[0.1, 0.5, 1, 1.5, 2.0, 2.5, 3, 3.5, 4.0, 4.5, 5.0]
# Subsequence length m is FIXED to the per-series default encoded in the file name
n_sigma = len(sigma)
...
for i_file, file in enumerate(file_list):
    ...
    m = int(match.group("sublen"))  # per-series default
    ...
    with out_file.open("wb") as f:
        np.savez(
            f, density=density, profile=profile, indices=indices, T=T, splice=splice
        )
    ...
    for i_sigma in range(len(sigma)):
        ...
        tau = tree.find_tau(
            k, m, density.T[i_sigma], indices.T[i_sigma], profile.T[i_sigma]
        )

        if tau is None:
            print(f"Not enough modes in {file.stem}, sigma={sigma[i_sigma]:.3f}")
            continue

        NNd, NNi, modes_idx, cluster_size = tree.tree2clusters(
            m, density.T[i_sigma], indices.T[i_sigma], profile.T[i_sigma], tau
        )

        # Unsupervised selection objective: the shift-invariant distance between the two modes
        mode_waves = utils.get_waves(np.sort(modes_idx).astype(np.int64), T, m)
        ...
        hits = 0
        # Same procedure as in snippet-finder/test.m
...
# For each file pick the sigma with the largest mode diversity, then read off whether that
# configuration was a success. Files where no sigma yielded k valid modes count as failures.
...
print(
    f"QSMP unsupervised-sigma success rate: "
