...
def rolling_window(a, window, splice=None):
    ...
    if splice is not None and splice.size > 0:
    ...
    for iteration in range(max_iter):
        try:
            ...
            for chunk in range(num_chunks):
                ...
                subseq = rolling_window(T[..., start:stop], m)
...
#TODO: Understand the trick here
#Ref:https://stackoverflow.com/a/34387987/4292705
def smooth(x, w):
    cumsum_vec = np.cumsum(np.r_[np.zeros(w//2), x, np.zeros(w//2)])
...
def fwhm(x):
    ...
    # Scale autocorrelation to [0,1] to specify peak prominence in
    # percentual units
    ...
    for i in range(n):
        ...
        if peaks.size == 1:
            to_the_left = x[i, :peaks[0]]
            left_min = np.min(to_the_left)
            half_val = (x[i, peaks[0]] - left_min)/2 + left_min
            left_half = (to_the_left <= half_val).nonzero()[0][-1]
            to_the_right = x[i, peaks[0]+1:]
            ...
            left_min = np.min(to_the_left)
            half_val = (x[i, peaks[i_max]] - left_min)/2 + left_min
            left_half = (to_the_left <= half_val).nonzero()[0][-1]
            left_half = left_half + peaks[i_max-1]
            to_the_right = x[i, peaks[i_max]+1:peaks[i_max+1]]
            right_half = (to_the_right <= half_val).nonzero()[0][0]
